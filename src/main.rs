use iter::{IndexedParallelIterator, IntoParallelRefIterator};
...
use rust3d::visualization::redering_object::{Mesh,Vertex};
...
fn main() {
    ...
    if let Some(mesh) = read_obj_file("./geometry/dk.obj"){
         ...
         let ratio_dk = 0.2;
         let trans_vector = (0.5, -0.5, 0.0); //translation vector.
         pre_process_obj(
            &(trans_vector.0),
            &(trans_vector.1),
            &(trans_vector.2),
